-- =============================================================================
-- HarnessOS — Postgres consolidated init (Aug 2026)
-- =============================================================================
--
-- Single-file bootstrap for the HarnessOS database on PostgreSQL.
-- Concatenates migrations 001–020 (000_combined.sql) + 024 (agents) +
-- 025 (memory, adapted to tsvector instead of SQLite FTS5).
--
-- Idempotent: every CREATE/INDEX uses IF NOT EXISTS, every column add
-- uses ADD COLUMN IF NOT EXISTS. Safe to re-run on an existing DB.
--
-- Required Postgres extensions (pre-installed on most managed Postgres):
--   - pgcrypto  (for gen_random_uuid())
--
-- Apply with:
--   PGPASSWORD="$DB_PASS" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" \
--     -d "$DB_NAME" -v ON_ERROR_STOP=1 -f deploy/postgres/harnessos-postgres-init.sql
--
-- =============================================================================
...
-- =============================================================================
-- Migrations 001–020 (consolidated, 21 tables + 4 Better Auth tables)
-- =============================================================================
...
-- ============================================================================
-- Migration 024 (agents) — already Postgres-friendly
-- ============================================================================
...
-- ============================================================================
-- Migration 025 (memory) — POSTGRES ADAPTATION
-- ============================================================================
--
-- SQLite-only bits removed:
--   * `id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16))))`
--     → `id UUID PRIMARY KEY DEFAULT gen_random_uuid()`
--   * `created_at TEXT NOT NULL DEFAULT (datetime('now'))`
--     → `created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()`
--   * `last_used_at TEXT` → `last_used_at TIMESTAMP WITH TIME ZONE`
--   * `CREATE VIRTUAL TABLE ... USING fts5(...)` + 3 triggers
--     → `content_tsv tsvector` column + trigger + GIN index
--       (Postgres equivalent of SQLite FTS5 with porter + unicode61)
--
-- This requires the @archon/core/db/memories.ts query path to be
-- Postgres-aware. See the companion patch:
--   deploy/postgres/memories-pg-patch.ts.diff
-- That patch adds a `isPostgres` branch that uses to_tsvector /
-- plainto_tsquery when DATABASE_URL points to Postgres, preserving
-- the FTS5 path for SQLite.
...
-- Postgres full-text search equivalent of SQLite FTS5 (porter + unicode61).
-- `simple` config keeps the diacritics-stripping behaviour we had with the
-- SQLite `unicode61 remove_diacritics 2` tokenizer; good enough for PT-BR +
-- EN phrases. Swap for a PT-BR stemmer later without schema changes.
ALTER TABLE remote_agent_memories
  ADD COLUMN IF NOT EXISTS content_tsv tsvector;

CREATE OR REPLACE FUNCTION remote_agent_memories_tsv_update() RETURNS trigger AS $$
BEGIN
...
DROP TRIGGER IF EXISTS memories_tsv_update ON remote_agent_memories;
CREATE TRIGGER memories_tsv_update
  BEFORE INSERT OR UPDATE OF content ON remote_agent_memories
  FOR EACH ROW EXECUTE FUNCTION remote_agent_memories_tsv_update();

-- Backfill any existing rows (no-op on fresh install).
...
COMMENT ON COLUMN remote_agent_memories.content_tsv IS
  'Postgres tsvector (simple config) — equivalent of SQLite FTS5 with porter + unicode61 remove_diacritics 2. Maintained by BEFORE INSERT/UPDATE trigger.';
COMMENT ON INDEX idx_memories_content_tsv IS
  'GIN index for fast @@ to_tsquery recall. Replaces the SQLite remote_agent_memories_fts virtual table.';

-- ============================================================================
-- End of HarnessOS Postgres init. Total: 21 + 4 (Better Auth) + 2 (agents) = 27 tables
-- (remote_agent_agents, remote_agent_agent_runs, remote_agent_memories — 3 new
--  beyond the 21 in 000_combined.sql; Better Auth adds 4; counting migration
--  024 separately gives a slightly different total — see README for the breakdown.)
-- ============================================================================
