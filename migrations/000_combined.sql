...
-- ============================================================================
-- 039: GitLab Integration — global org-level settings + issue ↔ demand links
--
-- Phase 1 (this migration): schema foundation only. The app code (Phase 2)
-- adds the settings store, GitLab REST client, admin settings UI, and
-- "test connection" endpoint.
--
-- Phases 3+ (issue board view, write sync, webhook receiver) reuse the
-- `gitlab_issue_links` table — no schema changes there.
-- ============================================================================

-- Org-level GitLab settings (singleton: PRIMARY KEY id = 1 enforced via
-- CHECK constraint; there is no per-user override at this scope because
-- the user explicitly asked for a global token).
CREATE TABLE IF NOT EXISTS remote_agent_gitlab_settings (
  id SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  -- e.g. "https://gitlab.com" or a self-hosted CE/EE URL. The REST v4
  -- API path is the same on both; we just prefix every request.
  gitlab_url TEXT NOT NULL,
  -- PAT (Personal Access Token) with `api` scope. Encrypted at rest
  -- with the same AES-256-GCM helper used for user_github_tokens and
  -- user_provider_keys.
  access_token_encrypted TEXT NOT NULL,
  -- Optional allowlist of "group/project" paths to expose in FORGE.
  -- NULL = all projects the token can see (most common). One entry per
  -- line; we keep it as a single TEXT column because the count is
  -- always in the dozens.
  project_filter TEXT,
  -- Master switch — when false, sync is paused but the credentials stay.
  sync_enabled BOOLEAN NOT NULL DEFAULT FALSE,
  -- Last successful "test connection" diagnostic surfaced in the admin UI.
...
-- Singleton row on first install (id=1). All other columns are NULL
-- / false until the admin saves the token from the Settings page.
...
CREATE TABLE IF NOT EXISTS remote_agent_gitlab_issue_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  demand_id UUID REFERENCES remote_agent_demands(id) ON DELETE SET NULL,
  -- GitLab's project_id (numeric) — derived from the encoded path on
  -- first resolve and cached here so we don't round-trip to GET /projects
  -- for every API call.
  ...
  -- project_id it uniquely identifies the issue. GitLab UI shows iid
  -- and the REST API accepts both.
  ...
  -- Last successful sync of this link (any direction).
  ...
  -- FORGE-side counter: demand.updated_at at the time of the last sync.
  ...
  -- issues) is allowed because a demand can span multiple GitLab
  -- issues (e.g. a backend issue + a frontend issue).
